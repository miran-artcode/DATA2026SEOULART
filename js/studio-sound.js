 ...
 * -----------------------------------------------------------------------------
 * 마이크 녹음 또는 오디오 파일(긴 것도 OK)에서 약 0.15초 프레임마다
 * 음량(RMS)·저/중/고·날카로움·변화·음높이(Hz)를 추출 → 시계열 데이터(CSV) → 데이터 점 스튜디오로.
...
(function () {
  ...
  const FEAT = {
    ...
    저음: ['#ff5a5f', '낮은 주파수 에너지 — 둥둥/우르렁'],
    중음: ['#ffb454', '가운데 주파수 — 말소리·멜로디 영역'],
    고음: ['#4ec3ff', '높은 주파수 — 쉰/치/반짝임'],
  ...
  function frameValues() {
    ...
    // 날카로움 = 스펙트럼 무게중심(에너지가 높은 주파수에 쓸릴수록 ↑)
  ...
  async function toggleMic() {
    ...
    } catch (e) { $('#rec-status').textContent = '마이크를 켜 수 없어요: ' + e.message; }
  }
  function stopMic() {
  ...
  function loadFile(file) {
    ...
    r.onload = async () => {
      try {
        ...
        // 긴 파일(예: 1시간)도 받게: 길수록 빨리 재생해 ~150프레임으로 압축 추출(짧은 건 거의 실시간).
