...
### 차시 순서 ≠ 단계 순서 (일부러 그렇게 두었음)

4단계 지도는 사진(2단계) → 소리(3단계)인데 학습지는 소리(4차시) → 삶(5차시)입니다.
학습지끼리 **‘다음 시간으로 넘기는 한 줄’**이 `s1→s2→s3→s4→s5→s6→s8`, `s7→s8` 로 이어져 있어서,
단계 순서대로 배치하면 아직 쓰지 않은 학습지의 한 줄을 앞 차시가 요구하게 됩니다.
그래서 **순차 진행의 척추는 차시 번호**로 두고, 각 차시에 단계 배지를 달았습니다.
